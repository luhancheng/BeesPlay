...
@interface CommonProblemVController ()<UITableViewDataSource, UITableViewDelegate>
...
@implementation CommonProblemVController
-(NSArray *)dataSource {
    if ( !_dataSource) {
        
        _dataSource = @[@[@"如何添加信息?",@"如何添加信息?"]];
    }
    return _dataSource ;
}
- (void)viewDidLoad {
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    if (indexPath.row == 0) {
        CommonProblemDetailVController *detailVC = [CommonProblemDetailVController new];
